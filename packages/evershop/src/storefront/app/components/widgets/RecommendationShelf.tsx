 ...
 * `{heading, limit}` settings shape — same here, just 4 registry entries.
 * Renders nothing off their anchor page (e.g. `related_products` placed on
 * the homepage has no current product) or when the resolver found nothing.
 */
export function RecommendationShelf({ widget, extra }: WidgetComponentProps) {
  const s = widget.rawSettings as { heading?: string | null };
